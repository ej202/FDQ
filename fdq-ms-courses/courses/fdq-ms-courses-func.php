<?php 
	...
	//--- INI :: CONTENT FUNCTIONS ------------------------------------------------------------------------------------------------------------
		...
		function fdq_ms_content_add_func($fv_title, $fv_icon, $fv_video_title, $fv_video_url, $fv_audio_title, $fv_audio_url, $fv_audio_url_male, $fv_audio_url_female, $fv_day, $fv_redir_url, $fv_active, $cod01){
			global $wpdb;
			$arr_validation = array();
            ...
            if( trim($fv_audio_url_male) != "" ):
				//$arr_validation[] = 'Audio Url is empty.';
				//else:
				if( !isWebsite($fv_audio_url_male) ):
					$arr_validation[] = 'Audio Url invalid data.';
				else:
					$fv_audio_url_male = sanitize_text_field($fv_audio_url_male);
                    $fv_audio_url_male = "'$fv_audio_url_male'";
				endif;
            else:
                $fv_audio_url_male = 'NULL';
			endif;

			if( trim($fv_audio_url_female) != "" ):
				//$arr_validation[] = 'Audio Url is empty.';
				//else:
				if( !isWebsite($fv_audio_url_female) ):
					$arr_validation[] = 'Audio Url invalid data.';
				else:
					$fv_audio_url_female = sanitize_text_field($fv_audio_url_female);
                    $fv_audio_url_female = "'$fv_audio_url_female'";
				endif;
            else:
                $fv_audio_url_male = 'NULL';
			endif;
			...
			if( count($arr_validation) < 1  and 1 == 1 ):
				...
				if( $res ):
					$qry = "INSERT INTO `wpou_mscm_content`( `course_pid`, `icon_pid`, `title`, `video_title`, `video_url`, `audio_title`, `audio_url`, `audio_url_male`, `audio_url_female`, `day_number`, `redir_url`, `cdate`, `active` )
						VALUES ('".$cod01."', '".$fv_icon."', '".$fv_title."', '".$fv_video_title."', '".$fv_video_url."', '".$fv_audio_title."', '".$fv_audio_url."', ".$fv_audio_url_male.", ".$fv_audio_url_female.", '".$fv_day."', '".$fv_redir_url."', '".$cdate."', '".$fv_active."')";// Notice male and female do not have single quotes, this is because they are set above already. This is to set them to NULL when they are empty. In the methods is_course_male_ready() and is_course_female_ready()  the check is for null and not empty string.
		...
		function fdq_ms_content_edit_func($fv_title, $fv_icon, $fv_video_title, $fv_video_url, $fv_audio_title, $fv_audio_url, $fv_audio_url_male, $fv_audio_url_female, $fv_day, $fv_redir_url, $fv_active, $cod01, $cod02){
			global $wpdb;
			$arr_validation = array();
			...
			if( count($arr_validation) < 1  and 1 == 1 ):
				...
				if( $res ):
					$qry = "UPDATE `wpou_mscm_content` SET
                            ...
                            `audio_url_male`= ".$fv_audio_url_male.",
                            `audio_url_female`= ".$fv_audio_url_female.",
							`day_number`='".$fv_day."',
							`redir_url`='".$fv_redir_url."',
							`active`= '".$fv_active."'
							WHERE content_id = '".$cod02."'";// Notice male and female do not have single quotes, this is because they are set above already. This is to set them to NULL when they are empty. In the methods is_course_male_ready() and is_course_female_ready()  the check is for null and not empty string.
