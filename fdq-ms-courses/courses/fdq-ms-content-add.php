<?php
	...
	if( trim($cod01) != "" ):
		if( isset($_REQUEST['btnAddContent']) ):
            ...
            $fv_audio_url_male = ( isset($_REQUEST['fv_audio_url_male']) && trim($_REQUEST['fv_audio_url_male']) != "" ) ? $_REQUEST['fv_audio_url_male'] : "" ;
            $fv_audio_url_female = ( isset($_REQUEST['fv_audio_url_female']) && trim($_REQUEST['fv_audio_url_female']) != "" ) ? $_REQUEST['fv_audio_url_female'] : "" ;
			...
			$err = fdq_ms_content_add_func($fv_title, $fv_icon, $fv_video_title, $fv_video_url, $fv_audio_title, $fv_audio_url, $fv_audio_url_male, $fv_audio_url_female, $fv_day, $fv_redir_url, $fv_active, $cod01);
		...
		//--- INI :: get icon data --------------------------------------------------------------
			$qryIcon = "SELECT * FROM `wpou_mscm_icon` WHERE active='1' ORDER BY icon_id";
	...
	<div class="mscm_main_wrapper">
		<?php if( $success_msg == "" ): ?>
			...
			<form name="recipe_add_form" id="recipe_add_form" action="<?php echo $pageUrl; ?>" method="post">
                ...
                <div class="mscm_row">
					<span class="mscm_field_title">Male Audio Url</span><br />
					<input type="text" name="fv_audio_url_male" id="fv_audio_url_male" value="<?php echo $fv_audio_url_male; ?>">
				</div>
                <div class="mscm_row">
					<span class="mscm_field_title">Female Audio Url</span><br />
					<input type="text" name="fv_audio_url_female" id="fv_audio_url_female" value="<?php echo $fv_audio_url_female; ?>">
				</div>
				<div class="mscm_row">
				...
				<div class="mscm_btn_row">
					<input type="hidden" name="action" value="showContentAdd" />
					<input type="hidden" name="cod01" value="<?php echo $cod01; ?>" />
					<input type="submit" name="btnAddContent" id="btnAddContent" value="Save Course Content" class="mscm_btn" />
			...
			<div class="mscm_successMsgWrapper">
				<div class="mscm_successMsgInner">
					...
					<div class="mscm_successMegButtons">
						<a class="mscm_btn" href="<?php echo $pageUrl; ?>&action=showContentAdd&cod01=<?php echo $cod01; ?>">Add Course Content</a>
						<a class="mscm_btn" href="<?php echo $pageUrl; ?>&action=showContentList&cod01=<?php echo $cod01; ?>">Cancel</a>
					</div>
				</div>
			</div>
		<?php endif; ?>
