...
var CirclePlayer = function(jPlayerSelector, media, options) {
	...
	this.player = jQueryP(jPlayerSelector);
	this.media = jQueryP.extend({}, media);
	this.options = jQueryP.extend(true, {}, defaults, options); // Deep copy
	...
	this.jq = {};
	jQueryP.each(cssSelector, function(entity, cssSel) {
		self.jq[entity] = jQueryP(self.options.cssSelectorAncestor + " " + cssSel);
	});
...
CirclePlayer.prototype = {
	...
	_initPlayer: function() {
		...
		this.player.bind(jQueryP.jPlayer.event.ready + this.eventNamespace, function(event) {
			if(event.jPlayer.html.used && event.jPlayer.html.audio.available) {
				self.audio = jQueryP(this).data("jPlayer").htmlElement.audio;
			}
			jQueryP(this).jPlayer("setMedia", self.media);
			self._initCircleControl(); //fabCode: comment this line to preven audio control 
		});

		this.player.bind(jQueryP.jPlayer.event.play + this.eventNamespace, function(event) {
			jQueryP(this).jPlayer("pauseOthers");
		});

		// This event fired as play time increments
		this.player.bind(jQueryP.jPlayer.event.timeupdate + this.eventNamespace, function(event) {
			if (!self.dragging) {
		...
		this.player.bind(jQueryP.jPlayer.event.progress + this.eventNamespace, function(event) {
			var percent = 0;
		...
		this.player.bind(jQueryP.jPlayer.event.ended + this.eventNamespace, function(event) {
			self._resetSolution();
		});
	},
	...
	_timeupdate: function(percent) {
		...
		if (percent <= 50) {
			...
			//if( percent >=97 && percent <= 100 ){ changeclosebtn(); }
