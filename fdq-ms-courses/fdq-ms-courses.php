...
/**
 * Edited by Muhammad Saqib Nadeem
 * Contact: https://www.freelancer.com/u/saqibhassan96
 * Email: m.saqib.hassan96@gmail.com
 */

add_action('wp_footer', 'fdq_ms_custom_footer_script', 999);

/**
 * Overwrites the updateStatus() function to redirect after clicking finish
 *
 * @return void
 */
function fdq_ms_custom_footer_script()
{
?>
    <script>
        jQuery(document).ready(function() {
            jQuery('.btnCheck').off('click');
            jQuery('body').on('click', '.btnCheck', function() {
                    ...
                    success: function(resp) {
                        ...
                        if (resp.step == '0') {
                            ...
                            if ((jQuery("." + boxObj).find('.a_' + resp.step).attr('data-info') != '') ||
                                (jQuery("." + boxObj).find('.a_' + resp.step).attr('data-name') != '')
                            ) { //if video or audio ok
                                step = resp.step;
                                jQuery("." + boxObj).find('.step').val(step);
                                img_cur = jQuery("." + boxObj).find('.i_' + step).attr(
                                    'data-info'); //current					     		 
...
function is_course_male_ready($course_pid){
    global $wpdb;

    // Get content_id male audio url is not set but the content is still active
    $q = "SELECT * FROM wpou_mscm_content WHERE course_pid='$course_pid' AND audio_url_male is NULL AND active = '1'";
    $results = $wpdb->get_results($q);
    if(count($results) > 0){
        // there is atleast one content where male url is missing hence the course is not male ready
        return false;
    }else{
        // All content have male url set, hence this course is male ready
        return true;
    }
}
function is_course_female_ready($course_pid){
    global $wpdb;

    // Get content_id female audio url is not set but the content is still active
    $q = "SELECT * FROM wpou_mscm_content WHERE course_pid='$course_pid' AND audio_url_female is NULL AND active = '1'";
    $results = $wpdb->get_results($q);
    if(count($results) > 0){
        // there is atleast one content where female url is missing hence the course is not female ready
        return false;
    }else{
        // All content have female url set, hence this course is female ready
        return true;
    }
}
function get_appropriate_audio_url($row, $gender){
    if($gender === 'M' && $row->audio_url_male ){
        return $row->audio_url_male;
    }
    if($gender === 'F' && $row->audio_url_female ){
        return $row->audio_url_female;
    }
    return $row->audio_url;
}


function fdq_ms_course_content_placeholder($atts)
{
    if (!session_id()) {
        session_start();
    }
    $_SESSION['last_short_code_atts'] = serialize($atts);
    ob_start();

?>
    <div class="fdq-ms-timeline-out-container">
<?php /*
        <div class="instructorSelectorContainer">
            <h4>Select Instructor: </h4>
            <!-- <div class="instructors-container"> -->
            <div class="instructor instructor-selected instructor-male" data-gender="male" onclick="changeInstructor('male')">
                <img src="https://dev5.mindfulscience.es/wp-content/uploads/2021/04/instructor-male.png"><span>Male</span>
            </div>
            <div class="instructor  instructor-female" data-gender="female" onclick="changeInstructor('female')">
                <img src="https://dev5.mindfulscience.es/wp-content/uploads/2021/04/instructor-female.png"><span>Female</span>
            </div>
            <!-- </div> -->
        </div>
*/ ?>

        <h3 align="center" class="inhalyexhale"> Inhala y exhala... </h3>

        <div class="TimeLinePlaceHolder"></div>
        <div class="fdq-ms-resetwrapper">
            <div class="box" style="display: none;">
                <a class="button btnShowPopup" href="#FDQResetCoursePopup">Restablecer el progreso del curso</a>
            </div>

            <div id="FDQResetCoursePopup" class="overlay" style="display:none">
                <div class="popup">
                    <div class="resetbox-content">
                        <h2><?php echo esc_html('¿Estás seguro?') ?></h2>
                        <a class="close btnHidePopup" href="#fdq-ms-resetwrapper">&times;</a>
                        <div class="content">
                            <p><?php echo esc_html('Esto restablecerá su progreso actual. Esta acción no se puede deshacer.') ?>
                            </p>
                            <p> <button class="fdq-ms-button-reset" onclick="fdqResetCourse()"><?php echo esc_html('Proceder a restablecer') ?></button>

                        </div>
                    </div>
                    <div class="resetting-content" style="display: none;">
                        <div class="content">
                            <?php echo esc_html('Restablecer el contenido del curso') ?>
                        </div>
                        <a class="close" href="#fdq-ms-resetwrapper">&times;</a>
                    </div>
                </div>
            </div>
        </div>
    ...
    <style>
        ...
        .instructorSelectorContainer .instructor {
            margin: 0px 10px;
            padding: 7px 20px;
            cursor: pointer;
            border-radius: 100px;
        ...
        .instructorSelectorContainer .instructor:hover {
            box-shadow: 4px 5px 15px 1px lightblue;
            background-color: mediumturquoise;
        }

        .instructorSelectorContainer .instructor-selected {
            background: #467592;
            color: white;
        }

        .instructorSelectorContainer .instructor img {
            border-radius: 100%;
            width: 50px;
            height: 50px;
            margin: 5px;
        ...
        .fdq-ms-resetwrapper .button:hover {
            background: #4e6d9c;
        }

        .fdq-ms-resetwrapper .overlay {
            position: fixed;
            top: 0;
            bottom: 0;
            left: 0;
        ...
        .fdq-ms-resetwrapper .popup h2 {
            margin-top: 0;
            color: #333;
            font-family: Tahoma, Arial, sans-serif;
        }

        .fdq-ms-resetwrapper .popup .close {
            position: absolute;
            top: 0px;
            right: 30px;
        ...
        .fdq-ms-resetwrapper .popup .close:hover {
            color: #06d85f;
        }

        .fdq-ms-resetwrapper .popup .content {
            max-height: 30%;
            overflow: auto;
        }

        .fdq-ms-resetwrapper .popup .fdq-ms-button-reset {
            background: none;
            color: firebrick
        }

        .fdq-ms-resetwrapper .popup .fdq-ms-button-reset:hover {
    ...
    <script>
        changeInstructor = function(gender) {
            jQuery('.instructor').removeClass('instructor-selected');
            jQuery('.instructor-' + gender).addClass('instructor-selected');
            initTimelineWrapper();
        }

        initTimelineWrapper = function() {
            var requestData = {
                action: "fdq_ms_load_timeline"
            }
            if(jQuery('.instructor-selected').length > 0 ){
                var selectedGenderType = jQuery('.instructor-selected').data('gender');
                requestData['gender'] = selectedGenderType;
            }
            ...
            jQuery.ajax({
                ...
                cache: false,
                beforeSend: function() {
                    console.log("Initiliazing Timeline now...");
                    jQuery('.inhalyexhale').show();
                    jQuery('.fdq-ms-resetwrapper .box').hide();
                    jQuery('.TimeLinePlaceHolder').html("");
                },
                success: function(resp) {
                    jQuery('.TimeLinePlaceHolder').html(resp);
                    jQuery('.fdq-ms-resetwrapper .box').show();

                    jQuery(".open-pop").on('click', function(e) {
                        e.preventDefault();
                        boxObj = jQuery(this).parent().attr('class');
                        ...
                        redirect_url = jQuery(this).attr('data-redirectUrl');

                        console.log('[open_id=' + open_id + '] [assig_id=' + assig_id + '] [step=' +
                            step + '] [laststep=' + laststep + '] [redirect_url=' + redirect_url +
                            ']');
                        if (parseInt(open_id) <= parseInt(step)) {
                            ...
                            if (str_video_url.trim() == '' && audio_url.trim() == '' && open_id ==
                                laststep) { //Nothing and last content
                                updateStatus();
                            } else {
                                if (str_video_url.trim() != '') {
                                    jQuery("#fdq-ms-vidHtml5Container").html(
                                        '<div class="resp-videowrapper"><iframe id="myvideo" src="' +
                                        str_video_url +
                ...
                error: function() {
                    jQuery('.TimeLinePlaceHolder').html("Error Loading Course...")
                },
                complete: function() {
                    jQuery('.inhalyexhale').hide();
                }
            });
        }

        fdqResetCourse = function() {
            assig_id = jQuery(".assig_id").val()
            data = {
                action: "fdq_ms_reset_course",
            ...
            jQuery.ajax({
                ...
                beforeSend: function() {
                    ...
                    jQuery('.fdq-ms-button-reset').prop('disabled', true);
                    jQuery('.resetbox-content').hide();
                    jQuery('.resetting-content').show();
                    jQuery('.resetting-content').html(
                        '<?php echo esc_html('Restablecer el contenido del curso') ?>');
                },
                success: function(resp) {
                    if (resp.success) {
                        console.log("Course resetting complete.");
                        jQuery('.resetting-content').html(resp.success);
                        setTimeout(() => {
                            jQuery('.fdq-ms-resetwrapper .overlay').fadeOut(500, function() {
                                jQuery('.resetbox-content').show();
                                jQuery('.resetting-content').hide();
                            });
                            initTimelineWrapper();
                        }, 1000);
                        return
                    ...
                    if (resp.error) {
                        jQuery('.resetting-content').html(resp.error);
                        setTimeout(() => {
                            jQuery('.fdq-ms-resetwrapper .overlay').fadeOut(500, function() {
                                jQuery('.resetbox-content').show();
                                jQuery('.resetting-content').hide();
                    ...
                    alert("Some unknown error occoured!");
                },
                error: function() {
                    alert('An error occoured. Please try later.');
                },
                complete: function() {
                    jQuery('.fdq-ms-button-reset').prop('disabled', false);
                }
            });
        }
        ...
        jQuery(window).ready(function() {
            initTimelineWrapper();
            jQuery('.fdq-ms-resetwrapper .btnShowPopup').on('click', function() {
                jQuery('#FDQResetCoursePopup').fadeIn()
            });
            jQuery('.fdq-ms-resetwrapper .btnHidePopup').on('click', function() {
                jQuery('#FDQResetCoursePopup').fadeOut()
            });
        });
    </script>
<?php
...
add_action('wp_ajax_nopriv_fdq_ms_load_timeline', 'fdq_ms_load_timeline');
add_action('wp_ajax_fdq_ms_load_timeline', 'fdq_ms_load_timeline');
function fdq_ms_load_timeline()
{
    /*
		* how to call this shortcode
		* [fdq-ms-course-content-show course_type="free"] ===> free course
    ...
    $atts = (isset($_SESSION['last_short_code_atts'])) ?  unserialize($_SESSION['last_short_code_atts']) : array();
    echo fdq_ms_course_content_display_func($atts);
    wp_die();
}

add_shortcode('fdq-ms-course-content-show', 'fdq_ms_course_content_placeholder');

add_action('wp_ajax_fdq_ms_reset_course', 'fdq_ms_reset_course');
function fdq_ms_reset_course()
{
    global $wpdb;
    $resp = array();
    $user_id = fdq_ms_get_user_id();
    $id = $_REQUEST['assignment_id'];


    if ($user_id == 0) {
        $resp['error'] = "User not logged in. Please login and try again.";
        wp_die(json_encode($resp));
    }

    if (!intval($id)) {
        $resp['error'] = "Invalid ID. Please try again later.";
        wp_die(json_encode($resp));
    }
    $res = $wpdb->get_results("SELECT * FROM wpou_mscm_assignment WHERE assignment_id =" . $id);
    $table = 'wpou_mscm_assignment';

    if ($user_id != $res[0]->user_pid) { // Verifying User
        $resp['error'] = "Invalid User!";
        wp_die(json_encode($resp));
    }

    if ($wpdb->update($table, array('finished' => '0', 'advanced' => '1'), array('assignment_id' => $id)) !== false) {
        $resp['success'] = esc_html("¡Ya puedes volver a repetir tu programa!");
        wp_die(json_encode($resp));
    } else {
        $resp['error'] = "No se pudo restablecer el programa. Por favor intente mas tarde.";
        wp_die(json_encode($resp));
    }
}

//add_action('wp_head', 'fdq_ms_add_popup_styling');
function fdq_ms_add_popup_styling()
{
?>
    <style>/*
        .fdq-ms-resetwrapper .box {
