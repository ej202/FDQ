<?php
	...
	if( isset($_REQUEST['btnEditsesmail']) ):
		...
		if( $cod01 != "" ):
			...
			$err = fdq_ms_sesmail_edit_func($fv_title, $fv_description, $fv_session_day, $fv_user_type, $fv_subject, $fv_msg, $fv_active, $cod01);
			if( count($err) > 0 ):
				$error_msg = setErrorMsg($err); //"Error: the recipe was not created";
				$success_msg = "";
			else:
				$success_msg = "The session mail was successfully updated";
				$error_msg = "";
			endif;
		endif;
		...
		$cod01 = ( isset($_REQUEST['cod01']) && trim($_REQUEST['cod01']) != "" ) ? $_REQUEST['cod01'] : "" ;
		if( $cod01 != "" ):
			$qry = "SELECT * FROM `wpou_mscm_sesmail` WHERE `sesmail_id` = '".$cod01."'"; //echo "qry: ".$qry."<br />";
			$rs = $wpdb->get_results($qry, ARRAY_A); //echo "<pre>";print_r($rs);echo "</pre>";
			$fv_title=$rs[0]['title'];
			$fv_description=$rs[0]['description'];
			$fv_session_day=$rs[0]['session_day'];
...
<div class="mscm_main_wrapper">
	<?php if( $cod01 != "" ): ?>
		<?php if( $success_msg == "" ): ?>
			...
			<div class="mscm_page_title">Edit session mail</div>
			...
			<div class="mscm_row">
				...
				<div class="mscm_btn_row">
					...
					<input type="hidden" name="action" value="showsesmailEdit" />
					<input type="submit" name="btnEditsesmail" id="btnEditsesmail" value="Edit Session Mail" class="mscm_btn" />
			...
			<div class="mscm_successMsgWrapper">
				<div class="mscm_successMsgInner">
					...
					<div class="mscm_successMegButtons">
						<a class="mscm_btn" href="<?php echo $pageUrl; ?>&action=showsesmailList">Go back</a>
					</div>
				</div>
			</div>
		<?php endif; ?>
