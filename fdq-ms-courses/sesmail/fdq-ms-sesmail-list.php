<?php
	...
	//--- INI :: email group activation/ deactivation / delete -----------------------------------------------------
		if( isset($_REQUEST['cod01']) and trim($_REQUEST['cod01']) != "" ):
			if( isset($_REQUEST['op']) ):
				$err = fdq_ms_sesmail_change_status_func($_REQUEST['cod01'], $_REQUEST['op']);
				if( count($err) > 0 ):
				...
				$err = fdq_ms_sesmail_del_func($_REQUEST['cod01']);
				if( count($err) > 0 ):
	...
	//-------------------------------------------------------------------------------------------------
	if( isset($_REQUEST['btnSesmailSearch']) ):
	...
	/*--- INI :: PAGINATION -----------------------------------------------------------*/
		...
		$sql = '';
		$sql .= "SELECT count(*) FROM wpou_mscm_sesmail ";
		$sql .= "WHERE 1 = 1 ";
		$sql .= $qry_title;
		$sql .= $qry_user_type;
		$sql .= $qry_active;
		$sql .= "ORDER BY sesmail_id";
		//echo "sql: ".$sql."<br />";
		...
		$lnk_firstpg = $pageUrl.'&pg='.$arr_pagination['firstpg'].$other_params."&btnSesmailSearch=ok";  //echo "lnk_firstpg: ".$lnk_firstpg."<br />";
		$lnk_prevpg  = $pageUrl.'&pg='.$arr_pagination['prevpg'].$other_params."&btnSesmailSearch=ok";   //echo "lnk_prevpg: ".$lnk_prevpg."<br />";
		$lnk_nextpg  = $pageUrl.'&pg='.$arr_pagination['nextpg'].$other_params."&btnSesmailSearch=ok";   //echo "lnk_nextpg: ".$lnk_nextpg."<br />";
		$lnk_lastpg  = $pageUrl.'&pg='.$arr_pagination['lastpg'].$other_params."&btnSesmailSearch=ok";   //echo "lnk_lastpg: ".$lnk_lastpg."<br />";
	/*--- END :: PAGINATION -----------------------------------------------------------*/

	/*--- INI :: BUILD MAIN QUERY -----------------------------------------------------*/
		$qry = '';
		$qry .= "SELECT * FROM wpou_mscm_sesmail ";
		$qry .= "WHERE 1 = 1 ";
		$qry .= $qry_title;
		$qry .= $qry_user_type;
		$qry .= $qry_active;
		$qry .= "ORDER BY sesmail_id ";
...
<div class="mscm_main_wrapper">
	...
	<div class="mscm_addNewWrapper">
		<p>Session Email: <a class="mscm_btn_add" href="<?php echo $pageUrl; ?>&action=showsesmailAdd">Add New</a></p>
	</div>
	<div class="mscm_search_wrapper">
		<form action="<?php echo $pageUrl; ?>" name="c_search_form" id="c_search_form" method="POST">
			...
			<div class="mscm_search_col04">
				<input type="hidden" name="action" value="showsesmailList">
				<input type="submit" name="btnSesmailSearch" id="btnSesmailSearch" value="search">
			</div>
		</form>
	</div>
	<div class="mscm_list_wrapper">
		<div class="mscm_list_title_wrapper">
			...
			<div class="mscm_list_title" style="width:10%">Session</div>
		...
		<?php if( count($rs) > 0 ): ?>
			<?php foreach( $rs as $val ): ?>
				...
				<div class="mscm_list_content_wrapper">
					...
					<?php else: ?>
						<div class="mscm_list_content <?php echo $_inact; ?>" style="width:22%"><?php echo "[".$val['sesmail_id']."]".$val['title']; ?></div>
					<?php endif;?>
					...
					<div class="mscm_list_content <?php echo $_inact; ?>" style="width:10%"><?php echo $val['session_day']; ?></div>
					...
					<div class="mscm_list_content <?php echo $_inact; ?>" style="width:16%">
						<a href="<?php echo $pageUrl; ?>&action=showsesmailEdit&cod01=<?php echo $val['sesmail_id']; ?>" title="Edit Email Group" >edit</a><br />
						<?php
							$_ac = '1';
							$_ac_label ='activate';
							if( $val['active'] == '1' ):
						...
						?>
						<a href="<?php echo $pageUrl; ?>&action=showsesmailList&cod01=<?php echo $val['sesmail_id']; ?>&op=<?php echo $_ac; ?>" title="<?php echo $_ac_label; ?> Content"><?php echo $_ac_label; ?></a><br /> 
						<a href="<?php echo $pageUrl; ?>&action=showsesmailList&cod01=<?php echo $val['sesmail_id']; ?>" onclick="return confirm('Are you sure you want to delete this?');" title="Deactivate Email Event">delete</a>
					</div>
				</div>
			<?php endforeach; ?>
		<?php else: ?>
