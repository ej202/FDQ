<?php
	...
	if( isset($_REQUEST['btnAddMgroup']) ):
		...
		$err = fdq_ms_mgroup_add_func($fv_title, $fv_description, $fv_absence_days, $fv_user_type, $fv_subject, $fv_msg, $fv_active);
		//$err = array('temp error');
		if( count($err) > 0 ):
			$error_msg = setErrorMsg($err); //"Error: the recipe was not created";
			$success_msg = "";
		else:
			$success_msg = "The new email group was successfully created";
			$error_msg = "";
		endif;
...
<div class="mscm_main_wrapper">
	<?php if( $success_msg == "" ): ?>
		...
		<div class="mscm_page_title">Add new Email Group</div>
		<form name="recipe_add_form" id="recipe_add_form" action="<?php echo $pageUrl; ?>" method="post">
			...
			<div class="mscm_btn_row">
				<input type="hidden" name="action" value="showMgroupAdd" />
				<input type="submit" name="btnAddMgroup" id="btnAddMgroup" value="Save Email Group" class="mscm_btn" />
		...
		<div class="mscm_successMsgWrapper">
			<div class="mscm_successMsgInner">
				...
				<div class="mscm_successMegButtons">
					<a class="mscm_btn" href="<?php echo $pageUrl; ?>&action=showMgroupAdd">Add Email Group</a>
					<a class="mscm_btn" href="<?php echo $pageUrl; ?>&action=showMgroupList">Cancel</a>
				</div>
			</div>
		</div>
	<?php endif; ?>
