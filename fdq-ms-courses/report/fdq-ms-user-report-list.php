<?php
	...
	if( $_SERVER['SERVER_NAME'] == 'locallab.dev' || $_SERVER['SERVER_NAME'] == 'www.mindfulscience.dev' || $_SERVER['SERVER_NAME'] == 'mindfulscience.dev' ):
		$user_tbl = 'wpou_users';
		$usermeta_tbl = 'wpou_usermeta';
		$capabilities = 'wpou_capabilities';
	else:
		$user_tbl = 'wp_users';
		$usermeta_tbl = 'wp_usermeta';
	...
	if( isset($_REQUEST['formdata']) && trim($_REQUEST['formdata']) != "" ):
		...
		if( $fv_autosubmit == 'y' ):
			...
			if( trim($fv_course) != "--" && trim($fv_session) != "--" && isNumber($fv_days) && $fv_days > 0 ):
				...
				$qry .= "DATE(a.last_visit) as last_login, TIMESTAMPDIFF(DAY,DATE(last_visit),CURDATE()) AS days ";
				$qry .= "FROM wpou_mscm_assignment a ";
	...
	//--- INI :: Built result_arr ----------------------------------------------------
		...
		if( $getResultOK ):
			...
			foreach($res as $value):
				...
				//--- INI :: get the role --------------------------------------------------------
					if( 1 == 2 ):
						$userInf = get_userdata( $value['ID'] );
						//echo "<pre>"; print_r($userInf->roles); echo "</pre>";
						echo "<hr />[".$value['ID']."] roles: ".implode(',', $userInf->roles)."<br />";
						//$roles = $userInf->roles;
						/*
						$_arr = array('8590', '10771', '190', '12076', '13165', '306', '13170', '257');
						if( in_array($value['ID'], $_arr) ):
							echo "<hr />roles: ".implode(',', $userInf->roles)."<br />";
						endif;
						/**/

						$role = '';
						if( in_array('contributor', $userInf->roles) ):
							$role = 'contributor';
						elseif(in_array('subscriber', $userInf->roles)):
							$role = 'subscriber';
						else:
							$role = implode(',', $userInf->roles);
						endif;
						//echo "<hr />role: ".$role."<br />";
					else:
						$qry01 = "SELECT * FROM `".$usermeta_tbl."` where meta_key = '".$capabilities."' and user_id = '".$value['ID']."'";
						//echo "qry: ".$qry01."<br />";
						$res01 = $wpdb->get_results($qry01, ARRAY_A);
						//echo "<pre>"; print_r($res01); echo "</pre>";
						$findme = 'subscriber';
						$mystring = $res01[0]['meta_value'];
	...
	//--- INI :: Generate CSV ---------------------------------------------------------
		if( isset($_REQUEST['btnGenCsv']) && count($result_arr) > 0 ):
			...
			$csv_arr_header = ["User_id","Registration Date","Role", "Name", "Email", "Course", "Session", "Last Login", "Days"];
			$csv_header = implode($csv_sep, $csv_arr_header);
			$csv_header = $csv_header.PHP_EOL;
			fputs($fp, $csv_header);

			foreach( $result_arr as $val ):
				$csv_row = $val['user_id'].$csv_sep.$val['rdate'].$csv_sep.$val['role'].$csv_sep.$val['name'].$csv_sep;
...
<div class="mscm_main_wrapper">
	...
	<div class="mscm_list_wrapper">
		<div class="mscm_list_title_wrapper">
			...
			<div class="mscm_list_title" style="width:9%">Role</div>
			<div class="mscm_list_title" style="width:20%">Name</div>
		...
		<?php if( count($result_arr) > 0 ): ?>
			...
			<?php while( $nu < $rowend && ($nu + $rowini) < $totalItems ): ?>
				...
				<div class="mscm_list_content_wrapper">
					...
					<div class="mscm_list_content <?php echo $_inact; ?>" style="width:9%"><?php echo $result_arr[$i]['role']; ?></div>
					<div class="mscm_list_content <?php echo $_inact; ?>" style="width:20%"><?php echo $result_arr[$i]['name']; ?></div>
					<div class="mscm_list_content <?php echo $_inact; ?>" style="width:20%"><?php echo $result_arr[$i]['email']; ?></div>
					<div class="mscm_list_content <?php echo $_inact; ?>" style="width:9%"><?php echo $result_arr[$i]['course']; ?></div>
