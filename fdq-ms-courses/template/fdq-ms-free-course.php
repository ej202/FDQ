...
if ($new_gender !== "" && $new_gender !== $gender) {
    $query_instructor_type_update = "UPDATE `wpou_mscm_assignment` SET instructor_type='$new_gender' WHERE assignment_id = '$assig_id'";

    if($wpdb->query($query_instructor_type_update)){
        $gender = $new_gender;
    }else{
        die("Error! Unable to change instructor, please reload page and try again...");
    }
}
...
?>
<style>
    <?php echo "." . $tlineWrapper ?> img {
        width: <?php echo $_iconwidth . "px"; ?>;
        height: auto;
    }
...
<?php
$male_ready = is_course_male_ready($course_pid);
$female_ready = is_course_female_ready($course_pid);

if ( $male_ready || $female_ready ) {
?>
    <div class="instructorSelectorContainer">
        <h4>Voz: </h4>
        <?php if ($male_ready) { ?>
            <div class="instructor <?php echo ($gender === 'M') ? 'instructor-selected' : '' ?> instructor-male" data-gender="male" onclick="changeInstructor('male')">
                <img src="https://dev5.mindfulscience.es/wp-content/uploads/2021/04/instructor-male.png"><span>Masculino</span>
            </div>
        <?php } //end if is_course_male_ready
        ?>

        <?php if ($female_ready) { ?>
            <div class="instructor <?php echo ($gender === 'F') ? 'instructor-selected' : '' ?> instructor-female" data-gender="female" onclick="changeInstructor('female')">
                <img src="https://dev5.mindfulscience.es/wp-content/uploads/2021/04/instructor-female.png"><span>Femenino</span>
            </div>
        <?php } //end if is_course_female_ready
        ?>
    </div>
<?php
}
...
<div class="timelineWrapper <?php echo $tlineWrapper ?> somerandomtestclass">
    <?php if ($finished == 1) : ?>
        ...
        <?php else : ?>
            ...
            <div class="icons_<?php echo $ran; ?>">
                ...
                $gvars = get_vars();
                $dir = $gvars['img_dir'];
                $qry03 = "SELECT * FROM wpou_mscm_content WHERE course_pid=" . $course_pid . " and active=1 order by day_number";
                ...
                foreach ($rs03 as $rowContent) :;
                    ...
                    endforeach;
                    // $audio = "";
                    $audio = get_appropriate_audio_url($rowContent, $gender);
                    if ($finished == 0) {
                        if (trim($audio) == '' && trim($rowContent->video_url) == '' && $c < $rowContent->count) {
                            //icon_blocked
                            ...
                            if ($advanced > $day_number) {
                                ...
                                echo    '	<span class="open-pop a_' . $day_number . '" 
                                            ...
                                            data-redirectUrl="' . $rowContent->redir_url . '"
